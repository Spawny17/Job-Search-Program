...
def totalJobs():
    website = requests.get("https://www.totaljobs.com/jobs/junior-developer/in-POST-CODE?radius=30&postedwithin=3").text
...
def indeedJobs():
    website = requests.get("https://www.indeed.co.uk/jobs?as_and=junior+developer&as_phr=&as_any=&as_not=&as_ttl=&as_cmp=&jt=all&st=&as_src=&salary=&radius=25&l=LOCATION+HERE&fromage=3&limit=20&sort=date&psf=advsrch").text
    soup = BeautifulSoup(website, 'lxml')
...
def reedJobs():
    website = requests.get("https://www.reed.co.uk/jobs/junior-developer-jobs-in-POSTCODE?datecreatedoffset=LastThreeDays&proximity=30").text
    soup = BeautifulSoup(website, 'lxml')
